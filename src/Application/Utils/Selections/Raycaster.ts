...
export default class Raycasting extends EventEmitter {
  ...
  private constructor(camera: Camera, scene: Scene, renderer: Renderer, objects: Group, selection: Selection) {
    super();
    this.camera = camera
    this.scene = scene
  ...
  static InitializeRaycaster(camera: Camera, scene: Scene, renderer: Renderer, objects: Group, selection: Selection) {
    return instance ?? new Raycasting(camera, scene, renderer, objects, selection)
  }
  ...
  private getMarkerPosition(intersection: Intersection, scale: number) {
