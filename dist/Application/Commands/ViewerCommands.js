...
export default class ViewerCommands {
    constructor(viewer, command) {
        this.onKeyDownEvent = (event) => {
            const isMac = navigator.platform.toUpperCase().includes('MAC');
            const key = event.key.toLowerCase();
            const isUndo = (key === 'z' && ((isMac && event.metaKey && !event.shiftKey) || (!isMac && event.ctrlKey && !event.shiftKey)));
            const isRedo = (key === 'z' && ((isMac && event.metaKey && event.shiftKey) || (!isMac && event.ctrlKey && event.shiftKey))) ||
    ...
    static initialize(viewer, command) {
    ...
    async zoomToFit(object) {
        var _a, _b;
        try {
            if (!((_b = (_a = this.viewer) === null || _a === void 0 ? void 0 : _a.activeCamera) === null || _b === void 0 ? void 0 : _b.controls)) {
                return;
            }
    ...
    addSection(box, normal, position) {
        this.command.executeCommand(new AddSectionCommand(box, normal, position, this.viewer.claddingObjectsGroup, this.viewer.sections, this.viewer.activeCamera, this.viewer.scene, this.viewer.renderer));
    }
    deleteSection(section) {
        this.command.executeCommand(new DeleteSectionCommand(section, this.viewer.claddingObjectsGroup, this.viewer.sections));
    }
    updateProperties(selectedObjs, config) {
